...
def prepareTrainValidateTest(trainRatio, valRatio):
  ...
  imagePathMio = dataPath / 'images_mio'
  imagesMio = [path for path in imagePathMio.glob('*.png')]
  imgDestPath = paths['images'] / 'train'
  labelDestPath = paths['labels'] / 'train'

  for img in imagesMio:
    label = Path(img).with_suffix('.txt')
    shutil.copy2(img, imgDestPath)
    shutil.copy2(label, labelDestPath)
